...
class InventoryController extends Controller
{
    ...
    public function createDiscrepancy()
    {
        ...
        $suppliersRegistry = is_array($registryData) ? $registryData : [];
        return view('inventory.discrepancy', compact('ledgeMap', 'allSuppliers', 'allDonors', 'existingItems', 'suppliersRegistry'));
    }
        ...
        try {
            ...
            if (!$is_admin) {
                ...
                if ($admins->count() > 0) {
                    ...
                    $msgContent .= "</div><div>";
                    $msgContent .= "<h4 style='margin: 0; color: #0f172a; font-size: 0.95rem; font-weight: 800; letter-spacing: -0.01em;'>DISCREPANCY ENTRY APPROVAL</h4>";
                    $msgContent .= "<p style='margin: 0; font-size: 0.75rem; color: #64748b; font-weight: 600;'>Pending Verification</p>";
                    $msgContent .= "</div></div>";
