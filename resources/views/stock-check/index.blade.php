        ...
        <div>
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                @if(auth()->user()->role === 'Auditor')
                    <span style="background: rgba(5, 150, 105, 0.1); color: #059669; font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase;">Audit Oversight Clearance</span>
                @elseif(in_array(auth()->user()->role, ['Main Admin', 'Department Head']))
                    <span style="background: rgba(5, 150, 105, 0.1); color: #059669; font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase;">{{ strtoupper(auth()->user()->department) }} Â· Department Head Hub</span>
                @else
                    <span style="background: rgba(5, 150, 105, 0.1); color: var(--primary); font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase;">Logistics Control</span>
                @endif
            </div>
            ...
            <table class="activity-table" style="width: 100%; min-width: 1000px; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $statusColor = $stock <= 0 ? '#ef4444' : ($stock <= $threshold ? '#059669' : '#059669');
                    ...
                    <tr class="activity-row" style="border-top: 1px solid var(--border-color);">
                        ...
                        <td data-label="Variance" style="padding: 1.25rem 1.5rem;">
                            <span style="font-weight: 800; color: {{ $variance > 0 ? '#059669' : ($variance < 0 ? '#ef4444' : '#94a3b8') }};">
...
<div id="stockCheckModal" class="modal-backdrop" style="display: none; align-items: center; justify-content: center; position: fixed; inset: 0 !important; width: 100vw !important; height: 100vh !important; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(10px); z-index: 99999 !important; transition: opacity 0.3s ease;">
    <div class="modal-content glass-card animate-scale-up" style="max-width: 800px; width: 95%; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 28px; box-shadow: 0 30px 70px rgba(0, 0, 0, 0.25); padding: 2rem; position: relative;">
        ...
        <div class="modal-body" style="display: flex; flex-direction: column; gap: 1.5rem;">
            {{-- Target Item Card --}}
            <div style="background: linear-gradient(135deg, rgba(5, 150, 105, 0.02) 0%, rgba(139, 92, 246, 0.02) 100%); padding: 1.5rem; border-radius: 24px; border: 1.5px solid var(--border-color); box-shadow: var(--shadow-premium);">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 1rem;">
                    <div class="audit-stat-card" style="background: rgba(5, 150, 105, 0.04); border-color: rgba(5, 150, 105, 0.18); border-radius: 16px; padding: 1rem; border-width: 1.5px; text-align: center;">
                    ...
                    <div class="audit-stat-card" style="border-radius: 16px; padding: 1rem; border: 1px solid rgba(5, 150, 105, 0.22); background: rgba(5, 150, 105, 0.02); text-align: center;">
                        <label style="color: #059669; font-size: 0.65rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 4px;">Active Loans</label>
                        <div id="auditActiveLoans" style="color: #059669; font-size: 1.6rem; font-weight: 950; line-height: 1.1;">0</div>
                    </div>
                </div>
                ...
                <div id="activeLoansDrawer" style="display: none; margin-top: 1.5rem; border-top: 1.5px solid var(--border-color); padding-top: 1.5rem; max-height: 250px; overflow-y: auto;">
                    <h4 style="font-size: 0.95rem; font-weight: 800; color: #059669; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 6px;">
            ...
            <form id="stockCheckForm" onsubmit="event.preventDefault(); {{ auth()->user()->role === 'Auditor' ? 'return false;' : 'submitStockCheck();' }}" style="display: flex; flex-direction: column; gap: 1.5rem;">
                ...
                <div class="audit-input-group" style="background: rgba(0, 0, 0, 0.01); padding: 1.75rem; border-radius: 24px; border: 1.5px solid var(--border-color);">
                    ...
                    <div id="auditInsightArea" style="margin-top: 1.25rem;">
                        <div class="insight-pill" id="auditInsight" style="background: rgba(5, 150, 105, 0.05); color: var(--primary); display: flex; align-items: center; gap: 8px; padding: 0.6rem 1.1rem; border-radius: 12px; font-size: 0.85rem; font-weight: 750; border: 1px solid rgba(5, 150, 105, 0.15);">
                ...
                <div style="display: grid; grid-template-columns: {{ auth()->user()->role === 'Auditor' ? '1fr' : '1.2fr 1fr' }}; gap: 1rem; margin-top: 1.5rem; align-items: center;">
                    @if(auth()->user()->role !== 'Auditor')
                    <button type="submit" class="btn-primary" style="width: 100%; padding: 1.1rem; border-radius: 18px; border: none; background: linear-gradient(135deg, #059669 0%, #059669 100%); color: white; font-weight: 900; font-size: 1rem; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 10px 25px rgba(5, 150, 105, 0.25);" onmouseover="this.style.transform='translateY(-1.5px)'; this.style.boxShadow='0 15px 30px rgba(5, 150, 105, 0.35)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 10px 25px rgba(5, 150, 105, 0.25)';" >
...
<div id="batchActionBar" style="position: fixed; bottom: 2rem; left: 50%; transform: translateX(-50%) translateY(150%); background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(12px); border: 1px solid rgba(0, 0, 0, 0.1); padding: 1rem 2rem; border-radius: 20px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.05); display: flex; align-items: center; gap: 2rem; z-index: 1000; transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); width: max-content; max-width: 90%; opacity: 0;">
    ...
    <button onclick="openBatchVerifyPage()" class="btn-primary" style="padding: 0.75rem 1.5rem; border-radius: 12px; border: none; background: linear-gradient(135deg, #059669 0%, #059669 100%) !important; color: white !important; cursor: pointer; font-weight: 800; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 14px rgba(5, 150, 105, 0.4);">
        <i data-lucide="shield-check" style="width: 18px; color: white !important;"></i>
            ...
            if (data.loans && data.loans.length > 0) {
                ...
                let html = `
                    <div style="overflow-x: auto; border-radius: 12px; border: 1.5px solid var(--border-color); background: var(--bg-main);">
                        <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 0.8rem;">
                            <thead>
                                <tr style="background: rgba(5, 150, 105, 0.04); border-bottom: 1px solid var(--border-color);">
                ...
                data.loans.forEach(loan => {
                    const overdueBadge = loan.is_overdue 
                        ...
                        : `<span style="font-size: 0.65rem; font-weight: 800; color: #059669; background: rgba(5, 150, 105, 0.08); padding: 0.15rem 0.4rem; border-radius: 4px; border: 1px solid rgba(5, 150, 105, 0.15);">Active</span>`;
                    html += `
...
function calculateAuditVariance() {
    ...
    varEl.style.color = variance === 0 ? '#059669' : (variance > 0 ? '#047857' : '#ef4444');
...
function generateVerificationReport() {
    ...
    const buildPdf = (imgEl) => {
        ...
        const secondaryColor = [59, 130, 246]; // #059669 (Blue)
        ...
        doc.text("NARCOTICS CONTROL COMMISSION Â· NACOC", 20, 16);
        ...
        doc.setFontSize(7.5);
        doc.setTextColor(...textMuted);
        doc.text("Page 1 of 1  Â·  NACOC Stores Logistics & Inventory Control System", 105, 282, { align: "center" });
...
function printAuditList() {
    ...
    const buildPdfList = (imgEl) => {
        ...
        const drawHeader = (isFirst) => {
            ...
            doc.text("NARCOTICS CONTROL COMMISSION Â· NACOC", 15, 13);
        ...
        const drawFooter = (totalPageCount) => {
            ...
            doc.setFontSize(7.5);
            doc.setTextColor(...textMuted);
            doc.text(`Page ${pageNum} of ${totalPageCount}  Â·  NACOC Stores Logistics & Inventory Control System`, 105, 281, { align: "center" });
        };
