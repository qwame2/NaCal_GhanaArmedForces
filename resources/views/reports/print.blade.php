...
<body>
    ...
    <div class="print-header">
        ...
        <div class="print-meta-bar">
            ...
            <div><strong>GENERATED BY:</strong> {{ auth()->user()->name }} ({{ auth()->user()->role }})</div>
            <div><strong>DATE &amp; TIME:</strong> {{ date('d M Y, h:i A') }}</div>
    ...
    <div class="report-generation-footer" style="margin-top: 40px; padding: 16px 20px; background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 6px; page-break-inside: avoid;">
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #334155; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            <div>
                <span style="font-weight: 800; text-transform: uppercase; color: #1e3a8a; letter-spacing: 0.05em; display: block; margin-bottom: 3px;">Report Generated By:</span>
                <strong style="font-size: 13px; color: #0f172a;">{{ auth()->user()->name }}</strong>
                <span style="color: #64748b; margin-left: 6px; font-weight: 600;">({{ auth()->user()->role }}{{ (auth()->user()->role !== 'Director General' && auth()->user()->department) ? ' &bull; ' . auth()->user()->department : '' }})</span>
            </div>
            <div style="text-align: right;">
                <span style="font-weight: 800; text-transform: uppercase; color: #1e3a8a; letter-spacing: 0.05em; display: block; margin-bottom: 3px;">Date &amp; Time Generated:</span>
