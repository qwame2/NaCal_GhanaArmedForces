...
@if(auth()->check() && (auth()->user()->role === 'Auditor' || auth()->user()->role === 'Main Admin' || auth()->user()->role === 'Sub Main Admin') && $batch->approval_status === 'pending_auditor_admin')
    ...
    <div class="review-banner" style="position: sticky; top: 0; left: 0; right: 0; background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(10px); color: #fff; padding: 15px 30px; display: flex; align-items: center; justify-content: space-between; z-index: 10000; box-shadow: 0 4px 20px rgba(0,0,0,0.3); border-bottom: 2px solid #334155; font-family: system-ui, -apple-system, sans-serif; margin-bottom: 20px;">
        <div style="display: flex; align-items: center; gap: 15px;">
            <div style="width: 42px; height: 42px; background: rgba(5, 150, 105, 0.2); border-radius: 12px; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(5, 150, 105, 0.4);">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#047857" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shield-alert"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>
            </div>
            <div style="text-align: left;">
                ...
                <div style="font-size: 0.78rem; color: #94a3b8; font-weight: 500; margin-top: 2px;">
                    @if($hasApproved)
                        <span style="color: #059669; font-weight: bold;">✓ You have approved this SRA.</span> Waiting for the other review authority.
                    @elseif($hasDeclined)
        ...
        @if(!$hasApproved && !$hasDeclined)
            <div style="display: flex; gap: 12px;">
                <button onclick="processSraReviewAction('approve')" style="background: #059669; color: white; border: none; padding: 10px 20px; border-radius: 10px; font-weight: 750; font-size: 0.85rem; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 4px 12px rgba(5, 150, 105, 0.25); transition: all 0.2s;" onmouseover="this.style.background='#065f46'" onmouseout="this.style.background='#059669'">
    ...
    <script>
        function processSraReviewAction(action) {
            ...
            const actionColor = action === 'approve' ? '#059669' : '#ef4444';
...
@endif
    ...
    <div class="sra-container">
        ...
        <div class="signatures-grid">
            ...
            <div class="sig-cell">
                <div class="sig-top-label" style="margin-bottom: 5px; text-align: center;">Taken on charge</div>
                <div class="sig-label">Storekeeper/Officer-in-Charge</div>
                <div class="sig-name-date">
                    <div><strong>Name:</strong> {{ $storesUser->name ?? '______________________' }}</div>
                    <div><strong>Role:</strong> 
                        @if(isset($storesUser))
                            {{ ($storesUser->role === 'Sub Main Admin' || $storesUser->role === 'Main Admin') ? ('Head of ' . preg_replace('/\s+department$/i', '', trim($storesUser->department ?: 'Stores')) . ' (Delegator Authorizer)') : ($storesUser->role ?? 'Head of Stores') }}
                        @else
                            {{ 'Head of Stores' }}
                        @endif
                    </div>
                    <div><strong>Date:</strong> {{ $storesDate ? \Carbon\Carbon::parse($storesDate)->format('d/m/y') : '______________________' }}</div>
                </div>
            </div>
            <div class="sig-cell">
                <div class="sig-top-label" style="margin-bottom: 5px; text-align: center;">Verified by</div>
                <div class="sig-label">Internal Audit/Stores Verifier</div>
                <div class="sig-name-date">
                    <div><strong>Name:</strong> {{ $auditorApproved ? $auditorUser->name : '______________________' }}</div>
                    <div><strong>Role:</strong> 
                        @if($auditorApproved && $auditorUser)
                        ...
                        @else
                            {{ $batch->auditor_status === 'approved' ? 'Auditor' : '______________________' }}
                        @endif
                    </div>
                    <div><strong>Date:</strong> {{ $auditorApproved && $batch->auditor_approved_at ? \Carbon\Carbon::parse($batch->auditor_approved_at)->format('d/m/y') : '______________________' }}</div>
                </div>
            </div>
        </div>
    </div>
