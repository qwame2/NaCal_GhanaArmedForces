...
<div class="animate-slide-up">
        ...
        <div>
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                @if(in_array(auth()->user()->role, ['Main Admin', 'Department Head', 'Auditor']))
                    <span style="background: rgba(5, 150, 105, 0.1); color: #059669; font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase;">{{ strtoupper(auth()->user()->department) }} · {{ auth()->user()->role === 'Auditor' ? 'Department Head' : 'Department Head Hub' }}</span>
                @else
                    <span style="background: rgba(5, 150, 105, 0.1); color: #059669; font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase;">Item Records</span>
                @endif
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
        <div class="glass-card" style="padding: 1.5rem; display: flex; align-items: center; gap: 1.25rem;">
            <div style="width: 48px; height: 48px; background: rgba(5, 150, 105, 0.1); color: var(--primary); border-radius: 12px; display: flex; align-items: center; justify-content: center;">
        ...
        <div class="glass-card" style="padding: 1.5rem; display: flex; align-items: center; gap: 1.25rem;">
            <div style="width: 48px; height: 48px; background: rgba(5, 150, 105, 0.1); color: #059669; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
    ...
    <div style="display: flex; gap: 1rem; align-items: center; margin-bottom: 3rem; flex-wrap: wrap;">
        <div style="display: flex; gap: 0.5rem; flex-shrink: 0;">
            ...
            <a href="{{ request()->fullUrlWithQuery(['status' => request('status') === 'partial' ? null : 'partial']) }}" style="padding: 0.65rem 1.4rem; border-radius: 999px; border: {{ request('status') === 'partial' ? '1.5px solid #059669' : '1.5px solid var(--border-color)' }}; background: {{ request('status') === 'partial' ? 'rgba(5, 150, 105, 0.1)' : 'var(--bg-card)' }}; color: {{ request('status') === 'partial' ? '#059669' : 'var(--text-main)' }}; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 8px; transition: all 0.3s; font-size: 0.85rem;">
            ...
            <a href="{{ request()->fullUrlWithQuery(['status' => request('status') === 'pending_approval' ? null : 'pending_approval']) }}" style="padding: 0.65rem 1.4rem; border-radius: 999px; border: {{ request('status') === 'pending_approval' ? '1.5px solid #059669' : '1.5px solid var(--border-color)' }}; background: {{ request('status') === 'pending_approval' ? 'rgba(5, 150, 105, 0.1)' : 'var(--bg-card)' }}; color: {{ request('status') === 'pending_approval' ? '#059669' : 'var(--text-main)' }}; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 8px; transition: all 0.3s; font-size: 0.85rem;">
            ...
            <a href="{{ request()->fullUrlWithQuery(['status' => request('status') === 'baseline' ? null : 'baseline']) }}" style="padding: 0.65rem 1.4rem; border-radius: 999px; border: {{ request('status') === 'baseline' ? '1.5px solid #059669' : '1.5px solid var(--border-color)' }}; background: {{ request('status') === 'baseline' ? 'rgba(5, 150, 105, 0.1)' : 'var(--bg-card)' }}; color: {{ request('status') === 'baseline' ? '#059669' : 'var(--text-main)' }}; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 8px; transition: all 0.3s; font-size: 0.85rem;">
            ...
            <!-- Store A Button -->
            <a href="{{ request()->fullUrlWithQuery(['store_location' => (request('store_location') === 'Store A' || request('store_location') === 'Stores A') ? null : 'Store A']) }}" style="padding: 0.65rem 1.4rem; border-radius: 999px; border: {{ (request('store_location') === 'Store A' || request('store_location') === 'Stores A') ? '1.5px solid var(--primary)' : '1.5px solid var(--border-color)' }}; background: {{ (request('store_location') === 'Store A' || request('store_location') === 'Stores A') ? 'rgba(5, 150, 105, 0.1)' : 'var(--bg-card)' }}; color: {{ (request('store_location') === 'Store A' || request('store_location') === 'Stores A') ? 'var(--primary)' : 'var(--text-main)' }}; font-weight: 800; text-decoration: none; display: flex; align-items: center; gap: 8px; transition: all 0.3s; font-size: 0.85rem;">
    ...
    <div id="resultsContainer" class="glass-card" style="overflow: visible; position: relative; border-radius: 20px;">
        ...
        <div style="background: var(--bg-card); border-radius: 20px 20px 0 0; border-bottom: 2px solid var(--primary); padding: 1.5rem 2rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; box-shadow: 0 4px 20px rgba(5, 150, 105, 0.05); z-index: 10;">
            <div style="display: flex; align-items: center; gap: 1.5rem;">
                <div style="width: 50px; height: 50px; background: rgba(5, 150, 105, 0.1); color: var(--primary); border-radius: 12px; display: flex; align-items: center; justify-content: center;">
            ...
            <div style="display: flex; gap: 2rem; align-items: center;">
                <div style="text-align: right;">
                    ...
                    <div style="font-size: 2rem; font-weight: 900; color: var(--text-main); line-height: 1;">
                        ...
                        <span style="color: #059669;">{{ number_format((float)($searchIssuedQtySum ?? 0)) }}</span> <span style="font-size: 1rem; color: var(--text-muted);">Qty Issued</span>
                        <span style="color: rgba(0,0,0,0.1); margin: 0 0.5rem;">|</span>
                    ...
                    @endphp
                        ...
                        <td data-label="Description" style="padding: 1.25rem 1.5rem;">
                            ...
                            <div style="font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; display: flex; align-items: center; gap: 6px;">
                                ...
                                <span style="font-size: 0.65rem; font-weight: 800; color: {{ str_contains($item->store_location ?? 'Store A', 'B') ? '#3b82f6' : 'var(--primary)' }}; background: {{ str_contains($item->store_location ?? 'Store A', 'B') ? 'rgba(59, 130, 246, 0.1)' : 'rgba(5, 150, 105, 0.1)' }}; padding: 1px 6px; border-radius: 4px; display: inline-flex; align-items: center; gap: 3px;">
                            ...
                            @if(!empty($item->serial_number))
                                ...
                                @if($count > 0)
                                    <div class="serial-numbers-wrapper" style="margin-top: 4px; display: inline-flex; flex-wrap: wrap; align-items: center; gap: 4px;">
                                        <div style="display: inline-flex; align-items: center; flex-wrap: wrap; gap: 4px; background: rgba(5, 150, 105, 0.08); color: var(--primary); font-size: 0.72rem; padding: 2px 8px; border-radius: 6px; font-weight: 800; word-break: break-word; white-space: normal; max-width: 250px;">
                        ...
                        <td data-label="Category" style="padding: 1.25rem 1.5rem;">
                            <span style="font-size: 0.75rem; background: rgba(5, 150, 105, 0.1); color: var(--primary); padding: 0.25rem 0.6rem; border-radius: 6px; font-weight: 600;">
                            ...
                            @if($expectedDates->isNotEmpty())
                                <div style="display: flex; flex-direction: column; gap: 4px; align-items: flex-start;">
                                    @foreach($expectedDates as $ed)
                                        ...
                                        @else
                                            <div style="display: flex; flex-direction: column; gap: 4px; align-items: flex-start;">
                                                <span style="font-size: 0.7rem; font-weight: 800; color: #065f46; background: #f0fdf4; border: 1.5px solid #bbf7d0; padding: 0.25rem 0.6rem; border-radius: 6px; text-transform: uppercase; white-space: nowrap; display: inline-flex; align-items: center; gap: 4px;" title="Expected Return Date">
                         ...
                         @if(!in_array(auth()->user()->role, ['Main Admin', 'Sub Main Admin']))
                             <td data-label="Action" style="padding: 1.25rem 1.5rem; text-align: right;">
                                 <div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px;">
                                     ...
                                     @endphp
                                     <button type="button" onclick="viewItemDetails('{{ $batchOrReqId }}', '{{ $item->id }}', {{ $isPending ? 'true' : 'false' }})" class="action-icon-btn" title="View Details" style="width: 38px; height: 38px; border-radius: 10px; color: #059669; background: rgba(5, 150, 105, 0.05); border: 1px solid rgba(5, 150, 105, 0.1); display: inline-flex; align-items: center; justify-content: center; cursor: pointer;">
                                     ...
                                     @if($isDbPartialDelivery && is_numeric($item->variance) && (float)$item->variance < 0)
                                         ...
                                         @endphp
                                         @if($pendingEdit)
                                             <div title="Edit Pending Approval" style="width: 38px; height: 38px; background: rgba(5, 150, 105, 0.08); color: #059669; border-radius: 10px; border: 1px dashed rgba(5, 150, 105, 0.3); display: flex; align-items: center; justify-content: center; opacity: 0.8; cursor: default;">
                                                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                                             </div>
                                         @else
                                             <button @if(in_array(auth()->user()->role, ['Main Admin', 'Sub Main Admin']) || (!auth()->user()->is_admin && !auth()->user()->can_add_inventory)) disabled title="Permission Denied" style="opacity: 0.4; cursor: not-allowed; width: 38px; height: 38px; border-radius: 10px; color: #111827; background: rgba(5, 150, 105, 0.05); border: 1px solid rgba(5, 150, 105, 0.1);" @else onclick="openEditBatchModal('{{ $item->batch_id }}')" class="action-icon-btn" title="Edit Entry" style="width: 38px; height: 38px; border-radius: 10px; color: #111827; background: rgba(5, 150, 105, 0.05); border: 1px solid rgba(5, 150, 105, 0.1);" @endif>
        ...
        <tr>
            <td colspan="{{ in_array(auth()->user()->role, ['Main Admin', 'Sub Main Admin']) ? 10 : 12 }}" style="padding: 10rem 2rem; text-align: center; vertical-align: middle;">
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1.5rem; margin: 0 auto;">
                    <div style="background: rgba(5, 150, 105, 0.05); width: 100px; height: 100px; border-radius: 30px; display: flex; align-items: center; justify-content: center; color: var(--primary); border: 2px dashed rgba(5, 150, 105, 0.2); animation: pulse 2s infinite;">
                    ...
                    <div style="max-width: 500px; text-align: center;">
                        ...
                        <div style="margin-top: 2.5rem; display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                            ...
                            <button onclick="window.location.href='/'" class="btn-primary" style="padding: 0.85rem 1.75rem; border-radius: 14px; border: none; font-size: 0.95rem; background: var(--primary-gradient); color: white; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 0.75rem; box-shadow: 0 12px 24px -6px rgba(5, 150, 105, 0.4); transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
...
<div id="viewBatchDetailsModal" class="modal-backdrop" style="display: none; z-index: 1050;">
    <div class="modal-content glass-card animate-scale-up" style="max-width: 650px; width: 95%; display: flex; flex-direction: column; max-height: 90vh; padding: 0; overflow: hidden; border: none; background: #ffffff; color: #000000; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25);">
        ...
        <div style="border-top: 1px solid #e2e8f0; display: flex; justify-content: flex-end; align-items: center; padding: 1.5rem 2.5rem; background: #f8fafc; border-radius: 0 0 28px 28px;">
            ...
            <button onclick="closeViewDetailsModal()" style="padding: 0.85rem 3rem; border-radius: 12px; font-weight: 800; font-size: 1rem; background: var(--primary); border: none; color: white; cursor: pointer; transition: all 0.3s; box-shadow: 0 8px 20px rgba(5, 150, 105, 0.25);">Close</button>
        </div>
    </div>
</div>
...
<div id="continueDeliveryModal" class="modal-backdrop">
    <div class="modal-content glass-card animate-scale-up" style="max-width: 800px; width: 95%;">
        <div class="modal-header">
            <div>
                <h3 style="margin: 0; font-size: 1.5rem; font-weight: 900; color: var(--text-main); display: flex; align-items: center; gap: 10px;">
                    <span style="width: 32px; height: 32px; background: rgba(5, 150, 105, 0.1); color: #059669; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
        ...
        <div style="border-top: 1px solid var(--border-color); display: flex; justify-content: flex-end; gap: 1rem; padding: 1.5rem; background: var(--bg-card); border-radius: 0 0 28px 28px;">
            ...
            <button onclick="submitContinueDelivery()" id="submitContinueBtn" style="padding: 0.85rem 1.5rem; border-radius: 12px; font-weight: 800; font-size: 0.95rem; background: #059669; border: none; color: white; cursor: pointer; display: flex; align-items: center; gap: 8px; box-shadow: 0 8px 20px rgba(5, 150, 105, 0.3); transition: all 0.3s;">
        ...
        <div class="modal-body">
            ...
            <div style="background: var(--bg-card); padding: 1.5rem; border-radius: 24px; margin-bottom: 1.5rem; border: 1px solid var(--border-color); box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    ...
                    <div class="audit-stat-card" style="border-color: rgba(5, 150, 105, 0.3); background: rgba(5, 150, 105, 0.02);" title="Package Types currently out on temporary loan">
                        <label style="color: #059669;">Active Loans</label>
                        <div id="auditActiveLoans" style="color: #059669;">0</div>
                    </div>
                </div>
            ...
            <form id="stockCheckForm" onsubmit="event.preventDefault(); submitStockCheck();" style="display: flex; flex-direction: column; gap: 1.5rem;">
                <div class="audit-input-group" style="background: var(--bg-main); padding: 1.5rem; border-radius: 20px; border: 1px solid var(--border-color);">
                    ...
                    <div id="auditInsightArea">
                        <div class="insight-pill" id="auditInsight" style="background: rgba(5, 150, 105, 0.05); color: var(--primary);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1.5fr; gap: 1rem;">
                    <div class="custom-select-wrapper">
                        ...
                        <select id="auditReason">
                            ...
                            <option value="Donor" data-icon="heart" data-color="#047857">Donation</option>
                    ...
                    <div style="grid-column: span 2;">
                        ...
                        <textarea id="auditNotes" placeholder="Describe the situation, location, or damage details in depth..." style="width: 100%; height: 80px; padding: 0.85rem; border: 1px solid var(--border-color); border-radius: 12px; background: var(--bg-card); color: var(--text-main); font-family: inherit; resize: none; outline: none; transition: all 0.3s;" onfocus="this.style.borderColor='var(--primary)'; this.style.boxShadow='0 0 0 4px rgba(5, 150, 105, 0.1)'" onblur="this.style.borderColor='var(--border-color)'; this.style.boxShadow='none'"></textarea>
                    </div>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1.5rem;">
                    <button type="submit" class="btn-primary" style="width: 100%; padding: 1.1rem; border-radius: 18px; border: none; background: var(--primary-gradient); color: white; font-weight: 900; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; box-shadow: 0 10px 20px rgba(5, 150, 105, 0.2); transition: all 0.3s;">
                ...
                <div id="reportComposerDrawer" style="display: none; margin-top: 2rem; border-top: 3px solid var(--primary); padding-top: 1.5rem; background: var(--bg-card); border-radius: 20px; padding: 1.5rem; border: 1px solid var(--border-color); box-shadow: 0 10px 40px rgba(0,0,0,0.05); animation: slideInUp 0.4s ease;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;">
                        ...
                        <span style="font-size: 0.65rem; color: var(--text-main); font-weight: 800; background: rgba(5, 150, 105, 0.1); padding: 4px 10px; border-radius: 100px;">Internal Verification Mode</span>
                    </div>
...
<style>
    ...
    :root {
        --primary-gradient: #059669;
        --success-gradient: #059669;
        --warning-gradient: #059669;
...
<script>
                ...
                if (itemsArray.length === 0) {
                    ...
                    itemsArray.forEach(item => {
                        ...
                        const rowStyle = isCurrentSelected 
                            ? 'background: rgba(5, 150, 105, 0.08); font-weight: bold; border-left: 3px solid var(--primary);' 
                            : '';
                        ...
                        let varianceHtml = '';
                        if (variance > 0) {
                            varianceHtml = `<span style="color: #059669; font-weight: 800;">+${Math.round(variance).toLocaleString()}</span>`;
                        } else if (variance < 0) {
                        ...
                        <div style="background: var(--bg-main); border: 1px solid var(--border-color); border-radius: 16px; padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem;">
                            ...
                            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; text-align: center; background: rgba(0,0,0,0.02); border-radius: 12px; padding: 1rem;">
                                ...
                                <div style="display: flex; flex-direction: column;">
                                    ...
                                    <span style="font-weight: 900; color: #059669; font-size: 1rem;">${stockBalance}</span>
                                </div>
                                <div style="display: flex; flex-direction: column;">
                            ...
                            <div id="recalc_preview_${item.id}" style="display: none; background: rgba(5, 150, 105, 0.05); border-left: 3px solid #059669; padding: 0.75rem 1rem; border-radius: 8px; margin-top: 0.5rem; font-size: 0.85rem;">
                    ...
                    if (!hasPending) {
                        html += `
                            <div style="text-align: center; padding: 3rem 1rem;">
                                <i data-lucide="check-circle" style="width: 48px; height: 48px; color: #059669; margin-bottom: 1rem;"></i>
    ...
    function previewRecalculation(itemId, expected, currentStock, currentQty, outstanding) {
        ...
        let varText = '';
        if (newVariance == 0) {
            varText = '<span style="color:#059669">Perfect match (No variance)</span>';
        } else if (newVariance < 0) {
            if (newQty >= expected) {
                varText = `<span style="color:#059669">Variance of ${newVariance} due to previously declared faults</span>`;
            } else {
                varText = `<span style="color:#ef4444">Still missing ${Math.abs(newVariance)}</span>`;
            }
        } else {
            varText = `<span style="color:#059669">Surplus of ${newVariance}</span>`;
        }
    ...
    function openStockCheckModal(description, ledgeBal, stockBal, prevVar, prevAvail) {
        ...
        if (insightPill) {
            ...
            insightPill.style.background = 'rgba(5, 150, 105, 0.05)';
    ...
    function calculateAuditVariance() {
        ...
        if (variance === 0) {
            ...
            indicatorFill.style.background = '#059669';
            insight.style.background = 'rgba(5, 150, 105, 0.1)';
            insight.style.color = '#059669';
    ...
    function renderAuditBreakdown(type) {
        ...
        auditHistoryData.forEach(item => {
            ...
            const sign = val > 0 ? '+' : '';
            const valColor = val < 0 ? '#ef4444' : (val > 0 ? '#059669' : 'var(--text-main)');
            ...
            html += `
                <div class="batch-audit-card" style="display: grid; grid-template-columns: auto 1fr auto; gap: 12px; align-items: start;">
                    ...
                    <div>
                        ...
                        ${item.remarks ? `
                            <div style="background: rgba(5, 150, 105, 0.03); padding: 0.5rem; border-radius: 8px; border-left: 2px solid var(--primary); font-size: 0.7rem; color: var(--text-muted); display: flex; align-items: flex-start; gap: 6px; margin-top: 4px;">
    ...
    async function generateAuditReport() {
        ...
        if (varValue === 0) {
            conclusion += `<div style="color: #059669;">SUCCESS: No variance detected. Physical stock perfectly aligns with system ledger records.</div>`;
        } else if (totalAccounted === parseFloat(stockBal)) {
            conclusion += `<div style="color: #059669;">BALANCED: System registry is balanced. Although physical stock is lower, all ${Math.abs(varValue)} missing package types are accounted for via active temporary loans.</div>`;
        } else if (totalAccounted < parseFloat(stockBal)) {
            ...
            conclusion += `<div style="color: #059669;">NOTICE: A surplus of ${varValue} package types has been discovered. Update requested for system ledger records to incorporate identified surplus.</div>`;
        }
            ...
            <html>
                ...
                <body>
                    ...
                    <div class="meta-grid" style="grid-template-columns: repeat(5, 1fr);">
                        ...
                        <div class="meta-item"><label>Active Loans (Temp)</label><div style="color: #059669;">${document.getElementById('auditActiveLoans').innerText}</div></div>
                    ...
                    <div class="audit-grid" style="margin-top: 20px;">
                        ...
                        <div>
                            ...
                            <div style="background: #f0fdf4; border: 1px solid #059669; padding: 15px; border-radius: 8px;">
                                ...
                                <div style="display: flex; justify-content: space-between; margin-bottom: 8px; border-bottom: 1px solid rgba(0,0,0,0.05);">
                                    ...
                                    <span style="font-weight: 900; color: #059669;">${document.getElementById('auditActiveLoans').innerText} Package Types</span>
                                </div>
                                <div style="display: flex; justify-content: space-between;">
                                    <span style="font-size: 0.7rem; font-weight: 800;">AUDIT VARIANCE:</span>
                                    <span style="font-weight: 900; color: ${parseFloat(variance) < 0 ? '#ef4444' : '#059669'};">${variance} Package Types</span>
                                </div>
                            </div>
...
<div id="editBatchModal" class="modal-backdrop">
    <div class="modal-content glass-card animate-scale-up" style="max-width: 1000px; width: 95%; padding: 0; overflow: hidden; border: none; background: #ffffff; display: flex; flex-direction: column; max-height: 90vh;">
        ...
        <div style="padding: 1.5rem 2.5rem; background: #059669; display: flex; justify-content: space-between; align-items: center; color: white;">
            <div>
                ...
                <div id="editModalContent" style="display: none;">
                    ...
                    <div style="margin-bottom: 2.5rem;">
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 1.5rem;">
                            <span style="font-size: 0.75rem; font-weight: 900; color: #059669; text-transform: uppercase; letter-spacing: 0.1em;">Received Details</span>
                    ...
                    <div>
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;">
                            <div style="display: flex; align-items: center; gap: 8px; flex: 1;">
                                <span style="font-size: 0.75rem; font-weight: 900; color: #059669; text-transform: uppercase; letter-spacing: 0.1em;">Item Information</span>
...
function promptActionReason(batchId, type = 'edit') {
    ...
    Swal.fire({
        ...
        confirmButtonColor: type === 'delete' ? '#ef4444' : '#059669',
...
function _openEditBatchModal(batchId, expiresIn = 62) {
    ...
    fetch(`{{ url('/received-items') }}/${batchId}?json=1`)
        ...
        .then(data => {
                ...
                const itemHtml = `
                    <div class="edit-item-card" data-id="${item.id}" style="background: #ffffff; padding: 1.5rem; border: 1.5px solid #f1f5f9; border-radius: 16px; transition: 0.3s; position: relative;">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 1.25rem;">
                            <div style="background: #eff6ff; color: #059669; width: 24px; height: 24px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 900;">${index + 1}</div>
                        ...
                        <div style="display: grid; grid-template-columns: 100px 1fr; gap: 1rem; align-items: flex-end;">
                            <div>
                                ...
                                <input type="number" class="item-variance" value="${item.variance}" readonly style="width: 100%; padding: 0.85rem; border: none; background: #f8fafc; border-radius: 12px; font-size: 0.85rem; font-weight: 900; color: ${item.variance < 0 ? '#ef4444' : '#059669'}; text-align: center;">
                            </div>
                            <div style="position: relative;">
            ...
            infoPanel.innerHTML = `
                <div style="margin-bottom: 1.5rem; padding: 1.25rem; background: rgba(5, 150, 105, 0.03); border: 1px dashed rgba(5, 150, 105, 0.2); border-radius: 16px; display: flex; justify-content: space-between; align-items: center;">
        ...
        const { value: text, isConfirmed } = await Swal.fire({
            html: `
                <div style="text-align: left;">
                    ...
                    <textarea id="swal-edit-justification" placeholder="e.g., Correcting a quantity typo, updating supplier info after delivery confirmation..." style="width: 100%; min-height: 110px; font-size: 0.9rem; border-radius: 14px; border: 2px solid #f1f5f9; padding: 1rem 1.25rem; font-family: inherit; resize: vertical; outline: none; transition: all 0.3s; box-sizing: border-box; color: #0f172a; background: #f8fafc;" onfocus="this.style.borderColor='#059669'; this.style.boxShadow='0 0 0 4px rgba(5, 150, 105,0.08)'" onblur="this.style.borderColor='#f1f5f9'; this.style.boxShadow='none'"></textarea>
            ...
            payload.items.forEach(item => {
                ...
                if (row) {
                    ...
                    if (varianceSpan) {
                        ...
                        varianceSpan.style.color = v > 0 ? '#059669' : (v < 0 ? '#ef4444' : '#94a3b8');
                    }
                }
            });
