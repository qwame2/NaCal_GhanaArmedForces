    ...
    <tbody>
        ...
        @if($req instanceof \App\Models\ServiceSra)
            @php
                ...
                $usageColor = '#881337';
            @endphp
            <tr class="req-table-row" data-type="sra" data-id="{{ $req->id }}" data-status="{{ $sraStatus }}" style="background: rgba(136, 19, 55, 0.015);">
                <td style="padding:1rem 1.5rem; text-align:center;">
                    <span style="background: rgba(136, 19, 55, 0.08); color: #881337; font-size: 0.75rem; font-weight: 800; padding: 5px 12px; border-radius: 99px; display: inline-block; white-space: nowrap;">
                        {{ $req->sra_number }}
                    </span>
                </td>
                ...
                <td style="padding:1rem 1.5rem;">
                    <div style="display:flex; flex-wrap:wrap; gap:6px;">
                        <span style="font-size: 0.72rem; font-weight: 700; color: var(--text-main); background: rgba(136,19,55,0.06); border: 1px solid rgba(136,19,55,0.15); padding: 4px 10px; border-radius: 8px; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap;">
                            <i data-lucide="wrench" style="width:12px;height:12px;color:#881337;"></i>
                ...
                <td style="padding:1rem 1.5rem; text-align:center;">
                    @if($req->status === 'approved' || $req->status === 'declined')
                        <button onclick="openSraOversightModal({{ $req->id }}, '{{ $sraStatus === 'admin_approved' ? 'stores' : 'admin' }}')"
                            style="background: rgba(136, 19, 55, 0.08); color: #881337; border: 1.5px solid rgba(136, 19, 55, 0.2); padding: 0.45rem 1rem; border-radius: 10px; font-weight: 800; cursor: pointer; font-size: 0.75rem; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s; white-space: nowrap;"
                            onmouseover="this.style.background='#881337'; this.style.color='white'; this.style.borderColor='#881337';"
                            onmouseout="this.style.background='rgba(136, 19, 55, 0.08)'; this.style.color='#881337'; this.style.borderColor='rgba(136, 19, 55, 0.2)';">
        ...
        @else
            ...
            <td style="padding:1rem 1.5rem; text-align:center;">
                <span style="background: rgba(234, 88, 12, 0.08); color: #4c0519; font-size: 0.75rem; font-weight: 800; padding: 5px 12px; border-radius: 99px; display: inline-block; white-space: nowrap;">
            ...
            <td style="padding:1rem 1.5rem;">
                <div style="display:flex; flex-wrap:wrap; gap:6px;">
                    ...
                    <span style="font-size: 0.72rem; font-weight: 700; color: var(--text-main); background: var(--bg-main); border: 1px solid var(--border-color); padding: 4px 10px; border-radius: 8px; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap;">
                        {{ Str::limit($item->description, 20) }} <span style="color:#4c0519; font-weight:800;">×{{ number_format($item->quantity_requested, 0) }}</span>
                    </span>
                    @endforeach
                    @if($req->items->count() > 3)
                    <span style="font-size: 0.72rem; font-weight: 700; color: #881337; background: rgba(136, 19, 55, 0.1); padding: 4px 10px; border-radius: 8px; white-space: nowrap;">+{{ $req->items->count() - 3 }} more</span>
                    @endif
                </div>
            </td>
            <td style="padding:1rem 1.5rem;">
