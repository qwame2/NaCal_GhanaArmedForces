...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav-menu">
            @if(auth()->user()->isItHeadOrStaff())
                <li class="nav-item">
                    <a href="{{ route('it-hub.dashboard') }}" class="nav-link {{ request()->routeIs('it-hub.dashboard') ? 'active' : '' }}" data-tooltip="IT Diagnostics Hub">
            ...
            @elseif(in_array(auth()->user()->role, ['Main Admin', 'Sub Main Admin', 'Department Head', 'Head of Stores', 'Dept. Head (Stores)']))
                ...
                @if(auth()->user()->role === 'Sub Main Admin' || auth()->user()->isDelegatedApprover())
                <li class="nav-item">
                    <a href="{{ route('admin.staff-approvals') }}" class="nav-link {{ request()->routeIs('admin.staff-approvals') ? 'active' : '' }}" data-tooltip="Staff Access &amp; Approvals">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-users"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        <span>Staff Access &amp; Approvals</span>
                    </a>
                </li>
                ...
                <li class="nav-item">
                    <a href="{{ route('stores.sra-vault') }}" class="nav-link {{ request()->routeIs('stores.sra-vault') ? 'active' : '' }}" data-tooltip="Approved SRA Vault">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-archive"><rect width="20" height="5" x="2" y="3" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"/><path d="M10 12h4"/></svg>
                        <span>Approved SRA Vault</span>
                    </a>
                </li>
                @endif
                @php
            ...
            @endif
                <li class="nav-item">
                    ...
                    @else
                    <a href="#" onclick="return false;" class="nav-link" style="opacity: 0.55; cursor: not-allowed;" data-tooltip="Stock Check is disabled for your account" data-tooltip-always="true">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="m9 14 2 2 4-4"/></svg>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.refreshUnreadMessages = function() {
                ...
                fetch("{{ route('api.total-unread', [], false) }}?_t=" + Date.now(), {
                    credentials: 'same-origin'
                })
                    .then(res => {
                        const contentType = res.headers.get("content-type");
        ...
        document.addEventListener('mouseover', (e) => {
            ...
            if (target) {
                ...
                if (sidebar) {
                    if (!sidebar.classList.contains('collapsed') && !target.hasAttribute('data-tooltip-always')) {
