...
<body>
            ...
            <div class="signatures-grid">
                ...
                <div class="sig-cell">
                    ...
                    <div>
                        <div class="sig-line" style="text-align: center;">
                            @php
                                $auditorUser = \App\Models\User::where('role', 'Auditor')->where('name', '!=', $sra->supplier_name)->first() ?? \App\Models\User::where('role', 'Auditor')->first();
                            @endphp
    ...
    <script>
        function handleStoresProcess(action) {
            Swal.fire({
                title: action === 'approved' ? 'Approve SRA?' : 'Decline SRA?',
                text: action === 'approved' 
                    ? 'This will finalize the Service SRA and update the system record.' 
                    : 'Add optional notes explaining why this SRA is being declined.',
                input: 'textarea',
                inputPlaceholder: 'Enter notes or remarks (optional)...',
                inputAttributes: {
                    'aria-label': 'Type your notes here'
                },
                icon: action === 'approved' ? 'question' : 'warning',
                showCancelButton: true,
                confirmButtonText: action === 'approved' ? 'Yes, Approve' : 'Decline SRA',
                ...
                .then(res => {
                    if (res.success) {
                        Swal.fire({
                            ...
                            setTimeout(() => {
                                window.location.href = '/admin/sra-history';
                            }, 500);
                        });
                    } else {
