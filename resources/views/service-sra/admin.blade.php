...
<style>
    ...
    .sra-table-row:hover { background: rgba(5,150,105,0.03); }
...
<div class="animate-slide-up">
    ...
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
        <div>
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                <span style="background: rgba(5,150,105,0.1); color: var(--primary); font-size: 0.7rem; font-weight: 800; padding: 0.25rem 0.75rem; border-radius: 9999px; text-transform: uppercase; letter-spacing: 0.05em;">Admin Review Queue</span>
                @if($pending->count() > 0)
    ...
    <div style="display: flex; gap: 0.5rem; border-bottom: 2px solid var(--border-color); margin-bottom: 1.5rem; padding-bottom: 2px;">
        <button onclick="switchAdminTab('pending')" id="tab-btn-pending" style="padding: 0.75rem 1.5rem; font-weight: 700; font-size: 0.88rem; border: none; background: transparent; cursor: pointer; border-bottom: 3px solid var(--primary); color: var(--primary); display: flex; align-items: center; gap: 8px; transition: all 0.2s;">
            ...
            <span style="background: rgba(5,150,105,0.12); color: var(--primary); padding: 2px 8px; border-radius: 99px; font-size: 0.72rem; font-weight: 800;">{{ $pending->count() }}</span>
        </button>
    ...
    <div id="admin-pane-pending" style="display: block;">
        <div class="glass-card" style="border-radius: 24px; overflow: hidden; padding: 0; margin-bottom: 2rem;">
            <div style="padding: 1.5rem 2rem; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; gap: 0.75rem;">
                <i data-lucide="clock" style="color: #059669; width: 20px;"></i>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ...
                            <tr class="sra-table-row" style="cursor: pointer;" onclick="openSraModal({{ $sra->id }})">
                                ...
                                <td style="padding: 1rem 1.5rem;">
                                    <span class="sra-badge" style="background: {{ $sra->delivery_type === 'full' ? 'rgba(5,150,105,0.1)' : 'rgba(5,150,105,0.1)' }}; color: {{ $sra->delivery_type === 'full' ? '#059669' : '#059669' }};">
                                ...
                                <td style="padding: 1rem 1.5rem; text-align: center;">
                                    <button onclick="event.stopPropagation(); openSraModal({{ $sra->id }})" style="background: rgba(5,150,105,0.08); color: var(--primary); border: 1px solid rgba(5,150,105,0.2); border-radius: 10px; padding: 0.5rem 1rem; font-size: 0.78rem; font-weight: 800; cursor: pointer; display: inline-flex; align-items: center; gap: 5px;">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ...
                            <tr class="sra-table-row">
                                ...
                                <td style="padding: 1rem 1.5rem;">
                                    <span class="sra-badge" style="background: {{ $sra->admin_status === 'approved' ? 'rgba(5,150,105,0.1)' : 'rgba(239,68,68,0.1)' }}; color: {{ $sra->admin_status === 'approved' ? '#059669' : '#ef4444' }};">
                                ...
                                <td style="padding: 1rem 1.5rem; text-align: center;">
                                    @if($sra->admin_status === 'approved')
                                        @if($sra->status === 'approved')
                                            <a href="{{ route('service-sra.receipt', $sra->id) }}" target="_blank" style="background: rgba(5,150,105,0.08); color: var(--primary); border: 1px solid rgba(5,150,105,0.2); border-radius: 10px; padding: 0.5rem 1rem; font-size: 0.78rem; font-weight: 800; text-decoration: none; display: inline-flex; align-items: center; gap: 5px;">
    ...
    <div class="modal-box" id="sraModalBox" onclick="event.stopPropagation()">
        ...
        <div id="modal-decision-form" style="border-top: 1px solid var(--border-color); padding-top: 1.5rem;">
            ...
            <div style="display: flex; gap: 1rem; margin-top: 1.25rem; justify-content: flex-end; flex-wrap: wrap;">
                ...
                <button onclick="processAdminSra('approved')" id="btnApprove" style="padding: 0.85rem 2rem; border: none; background: #059669; color: white; border-radius: 12px; cursor: pointer; font-weight: 800; font-size: 0.9rem; display: flex; align-items: center; gap: 0.5rem; box-shadow: 0 8px 20px -5px rgba(5,150,105,0.4);">
