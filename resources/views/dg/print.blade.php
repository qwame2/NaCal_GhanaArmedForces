...
<head>
    ...
    <title>Director General Oversight & Command Ledger - NACOC</title>
    <style>
        body {
...
<body onload="window.print();">
    ...
    <div class="header-container">
        ...
        <div class="doc-title">Director General Command Oversight & Verification Report</div>
    </div>
    ...
    <div class="meta-grid">
        <div>
            ...
            <div class="meta-item"><span>Department:</span> {{ $dg->role === 'Director General' ? 'STORES' : ($dg->department ?? 'Office of the Director General') }}</div>
        ...
        <div style="text-align: right;">
            ...
            <div class="meta-item"><span>Oversight Classification:</span> STRICTLY CONFIDENTIAL - DIRECTOR GENERAL ONLY</div>
        </div>
    </div>
        ...
        <tbody>
            @forelse($receivedItems as $item)
                <tr>
                    ...
                    <td>
                        ...
                        @if(!empty($item->serial_number))
                            <div style="font-size: 8px; color: #475569; margin-top: 2px;">
                                S/N: <code style="font-family: monospace;">{{ $item->serial_number }}</code>
                            </div>
                        @endif
                        @if(!empty($item->remarks))
                            <div style="font-size: 8px; color: #64748b; margin-top: 2px; font-style: italic;">
                                {{ $item->remarks }}
                            </div>
                        @endif
                    </td>
                    ...
                    <td style="text-align: right; font-weight: 700; color: #059669;">{{ number_format($item->stock_balance) }} {{ $item->unit }}</td>
                    ...
                    <td>
                        @if($item->supplier_name)
                            ...
                            @if($supDetails)
                                <div style="font-size: 8px; color: #475569; margin-top: 3px; line-height: 1.3;">
                                    @if(!empty($supDetails['phone'])) Phone: {{ $supDetails['phone'] }}<br> @endif
                                    @if(!empty($supDetails['email'])) Email: {{ $supDetails['email'] }}<br> @endif
                                    @if(!empty($supDetails['address'])) Address: {{ $supDetails['address'] }}<br> @endif
                                    @if(!empty($supDetails['delivery_person'])) Rep: {{ $supDetails['delivery_person'] }} @if(!empty($supDetails['delivery_phone'])) ({{ $supDetails['delivery_phone'] }}) @endif @endif
                                </div>
                            @endif
                        @elseif($item->donor_name)
                            <strong>{{ $item->donor_name }}</strong><br>
                            <span style="font-size: 8px; color: #64748b;">(Donor Contribution)</span>
                        @else
    ...
    <div class="report-generation-footer" style="margin-top: 40px; padding: 16px 20px; background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 6px; page-break-inside: avoid;">
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #334155;">
            <div>
                <span style="font-weight: 800; text-transform: uppercase; color: #0f172a; letter-spacing: 0.05em; display: block; margin-bottom: 3px;">Report Generated By:</span>
                <strong style="font-size: 13px; color: #0f172a;">{{ $dg->name }}</strong>
                <span style="color: #64748b; margin-left: 6px; font-weight: 600;">({{ $dg->role }}{{ ($dg->role !== 'Director General' && $dg->department) ? ' &bull; ' . $dg->department : '' }})</span>
            </div>
            <div style="text-align: right;">
                <span style="font-weight: 800; text-transform: uppercase; color: #0f172a; letter-spacing: 0.05em; display: block; margin-bottom: 3px;">Date &amp; Time Generated:</span>
                <strong style="font-size: 12px; color: #0f172a;">{{ date('d M Y, h:i A') }}</strong>
            </div>
        </div>
    </div>
