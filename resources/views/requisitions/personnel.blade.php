...
<div style="padding:2rem;">
    ...
    <div style="margin-bottom:2rem; display:flex; align-items:flex-start; justify-content:space-between; gap:1rem; flex-wrap:wrap;">
        <div>
            <div style="font-size:.7rem;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.12em;margin-bottom:4px;">Store Operations</div>
        ...
        <button onclick="openNewReqPanel()" id="new-req-btn"
            style="display:inline-flex;align-items:center;gap:8px;padding:.75rem 1.5rem;background:#059669;color:white;border:none;border-radius:14px;font-weight:800;font-size:.875rem;cursor:pointer;box-shadow:0 4px 15px rgba(5, 150, 105,.3);transition:all .25s cubic-bezier(.16,1,.3,1);flex-shrink:0;"
            onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 25px rgba(5, 150, 105,.4)'"
            onmouseout="this.style.transform='';this.style.boxShadow='0 4px 15px rgba(5, 150, 105,.3)'">
                ...
                <tr class="req-table-row" data-req-id="{{ $req->id }}" data-status="{{ $req->status }}" data-collected="{{ $req->collected_at ? '1' : '0' }}">
                    ...
                    <td style="padding:1rem 1.5rem;">
                        <div style="display:flex;flex-wrap:wrap;gap:4px;">
                            ...
                            <span style="font-size:.7rem;font-weight:700;color:#059669;background:rgba(5, 150, 105,.1);padding:2px 8px;border-radius:6px;">+{{ $req->items->count()-3 }} more</span>
                            @endif
                        </div>
                    </td>
                    ...
                    <td style="padding:1rem 1.5rem;text-align:center;">
                        @if(in_array($req->status, ['approved', 'partially_approved']))
                            @if($req->collected_at)
                            <span style="font-size:.78rem;color:#059669;font-weight:800;display:inline-flex;align-items:center;gap:4px;">
                        ...
                        @else
                        <span style="font-size:.75rem;color:#ef4444;font-style:italic;font-weight:700;">Awaiting Head of Stores Review</span>
                        @endif
                    </td>
                    <td style="padding:1rem 1.5rem;text-align:center;">
                        <button onclick="openRequisitionModal({{ $req->id }})"
                            style="background:rgba(5, 150, 105,.1);color:#059669;border:none;padding:.5rem 1rem;border-radius:10px;font-weight:800;font-size:.78rem;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:.15s;" onmouseover="this.style.background='#059669';this.style.color='white'" onmouseout="this.style.background='rgba(5, 150, 105,.1)';this.style.color='#059669'">
                ...
                <div style="display: flex; gap: 0.25rem;">
                    @foreach($requisitions->appends(request()->query())->getUrlRange(max(1, $requisitions->currentPage()-2), min($requisitions->lastPage(), $requisitions->currentPage()+2)) as $page => $url)
                        @if($page == $requisitions->currentPage())
                            <span style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 8px; background: var(--primary); color: white; font-size: 0.85rem; font-weight: 900; box-shadow: 0 4px 10px rgba(5, 150, 105,0.2);">{{ $page }}</span>
                        @else
        ...
        <div style="padding:1.5rem 2rem;border-bottom:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
            <div style="display:flex;align-items:center;gap:1rem;">
                <div style="width:44px;height:44px;background:rgba(5, 150, 105,.1);border-radius:12px;display:flex;align-items:center;justify-content:center;">
                    <i data-lucide="clipboard-list" style="width:20px;color:#059669;"></i>
                </div>
                <div>
...
<div id="newReqOverlay" onclick="if(event.target===this)closeNewReqPanel()" style="position:fixed;inset:0;width:100vw;height:100vh;background:rgba(15,23,42,.5);backdrop-filter:blur(6px);z-index:99998;display:none;align-items:flex-start;justify-content:flex-end;">
    <div id="newReqPanel" style="background:var(--bg-card);width:100%;max-width:1100px;height:100vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:-30px 0 80px rgba(15,23,42,.18);transform:translateX(100%);transition:transform .4s cubic-bezier(.16,1,.3,1);">
        ...
        {{-- Panel Header --}}
        <div style="padding:1.5rem 2rem;border-bottom:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;background:rgba(5,150,105,0.04);">
            <div style="display:flex;align-items:center;gap:1rem;">
                <div style="width:44px;height:44px;background:#059669;border-radius:12px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(5, 150, 105,.3);">
        ...
        <div style="padding:1rem 2rem;border-bottom:1px solid var(--border-color);flex-shrink:0;">
            <div style="display:flex;align-items:center;">
                ...
                <div style="flex:1;height:2px;background:var(--border-color);margin:0 .75rem;border-radius:2px;position:relative;">
                    <div id="nr-progress-1" style="position:absolute;left:0;top:0;height:100%;width:0;background:#059669;border-radius:2px;transition:width .4s;"></div>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                ...
                <div style="flex:1;height:2px;background:var(--border-color);margin:0 .75rem;border-radius:2px;position:relative;">
                    <div id="nr-progress-2" style="position:absolute;left:0;top:0;height:100%;width:0;background:#059669;border-radius:2px;transition:width .4s;"></div>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="flex:1;overflow-y:auto;padding:1.75rem 2rem;" class="modal-body">
            ...
            <div id="nr-step-1">
                <div style="margin-bottom:1.5rem;">
                    <div style="font-size:.7rem;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.1em;margin-bottom:1rem;">Requester Information</div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:6px;">
                            ...
                            <input type="text" id="nr-rank" value="{{ auth()->user()->rank_or_title ?? '' }}" placeholder="e.g. Sergeant, Officer" style="padding:.75rem 1rem;border:1.5px solid var(--border-color);border-radius:10px;background:var(--bg-main);color:var(--text-main);font-family:inherit;font-size:.88rem;font-weight:600;outline:none;transition:.2s;" onfocus="this.style.borderColor='#059669';this.style.boxShadow='0 0 0 4px rgba(5, 150, 105,.12)'" onblur="this.style.borderColor='var(--border-color)';this.style.boxShadow=''">
                        </div>
                        <input type="hidden" id="nr-priority" value="normal">
                    </div>
                </div>
                <div style="margin-bottom:1.5rem;">
                    <div style="font-size:.7rem;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.1em;margin-bottom:1rem;">Usage Type <span style="color:#ef4444;">*</span></label>
                    <div style="display:flex;gap:1rem;">
                        <label style="flex:1;display:flex;align-items:center;gap:.75rem;padding:1rem;border:1.5px solid #059669;background:rgba(5, 150, 105,.04);border-radius:12px;cursor:pointer;transition:.2s;" id="nr-usage-perm-label" onclick="selectNrUsage('permanent')">
                            <input type="radio" name="nr_usage" id="nr-usage-permanent" value="permanent" checked style="accent-color:#059669;width:16px;height:16px;">
                            <div>
                        ...
                        <label style="flex:1;display:flex;align-items:center;gap:.75rem;padding:1rem;border:1.5px solid var(--border-color);border-radius:12px;cursor:pointer;transition:.2s;" id="nr-usage-temp-label" onclick="selectNrUsage('temporary')">
                            <input type="radio" name="nr_usage" id="nr-usage-temporary" value="temporary" style="accent-color:#059669;width:16px;height:16px;">
                            <div>
                ...
                <div style="display:grid;grid-template-columns:1fr 420px;gap:1.25rem;height:calc(100vh - 320px);min-height:420px;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:.5rem;background:var(--bg-main);border:1.5px solid var(--border-color);border-radius:14px;padding:.85rem;overflow:hidden;">
                        <div style="font-size:.68rem;font-weight:800;color:var(--text-muted);text-transform:uppercase;letter-spacing:.06em;display:flex;align-items:center;justify-content:space-between;">
                            ...
                            <span id="nr-cart-count" style="background:#059669;color:white;font-size:.65rem;font-weight:900;padding:2px 7px;border-radius:8px;">0</span>
                        </div>
            ...
            <div id="nr-step-3" style="display:none;">
                <div style="font-size:.7rem;font-weight:800;color:#059669;text-transform:uppercase;letter-spacing:.1em;margin-bottom:1rem;">Review Your Requisition</div>
        ...
        <div style="padding:1.25rem 2rem;border-top:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-shrink:0;background:var(--bg-card);">
            ...
            <button id="nr-btn-next" onclick="nrGoNext()" style="padding:.75rem 1.75rem;border:none;border-radius:12px;background:#059669;color:white;font-weight:800;font-size:.85rem;cursor:pointer;display:inline-flex;align-items:center;gap:8px;box-shadow:0 4px 12px rgba(5, 150, 105,.25);transition:.2s;">
                Next <i data-lucide="arrow-right" style="width:15px;"></i>
            </button>
            <button id="nr-btn-submit" onclick="submitNewReq()" style="display:none;padding:.75rem 1.75rem;border:none;border-radius:12px;background:#059669;color:white;font-weight:800;font-size:.85rem;cursor:pointer;align-items:center;gap:8px;box-shadow:0 4px 12px rgba(5, 150, 105,.25);transition:.2s;">
...
.nr-step-label.active { color:#059669; }
.nr-step-label.done { color:#059669; }
</style>
...
<script>
            ...
            <div class="item-card-panel" style="gap:1.5rem;">
                ...
                <div style="flex:2; min-width:180px;">
                    ...
                    <div class="fulfill-progress-container" style="margin-top:0;">
                        <div class="fulfill-progress-bar" style="width: ${pct}%; background:${approved === 0 ? '#ef4444' : '#059669'}"></div>
                    </div>
                </div>
            </div>
    ...
    ${data.admin_notes ? `
        <div style="background:rgba(5, 150, 105,.03);border:1px solid rgba(5, 150, 105,.15);border-radius:16px;padding:1.25rem; margin-top: 1.25rem;">
    ...
    function nrGoNext() {
        if (nrCurrentStep === 1) {
            ...
            if (!name || !dept || !purpose) {
                Swal.fire({ icon:'warning', title:'Missing Fields', text:'Please fill in Full Name, Department, and Purpose.', confirmButtonColor:'#059669' });
                return;
            }
        } else if (nrCurrentStep === 2) {
            if (nrCartItems.length === 0) {
                Swal.fire({ icon:'warning', title:'No Items Added', text:'Please add at least one item to your requisition.', confirmButtonColor:'#059669' });
                return;
            }
            const invalidQty = nrCartItems.find(i => !i.qty || parseFloat(i.qty) <= 0);
            if (invalidQty) {
                Swal.fire({ icon:'warning', title:'Invalid Quantity', text:`Please enter a valid quantity for: ${invalidQty.description}`, confirmButtonColor:'#059669' });
                return;
            }
            nrBuildReview();
        }
    ...
    function selectNrPriority(val) {
        ...
        document.querySelectorAll('.nr-priority-btn').forEach(btn => {
            ...
            if (v === val) {
                ...
                else if (v === 'normal') { btn.style.borderColor='#059669'; btn.style.background='rgba(5, 150, 105,.08)'; btn.style.color='#059669'; }
    ...
    function selectNrUsage(val) {
        ...
        if (val === 'permanent') {
            permLabel.style.borderColor = '#059669'; permLabel.style.background = 'rgba(5, 150, 105,.04)';
            tempLabel.style.borderColor = 'var(--border-color)'; tempLabel.style.background = '';
        } else {
            tempLabel.style.borderColor = '#059669'; tempLabel.style.background = 'rgba(5, 150, 105,.04)';
    ...
    function nrBuildCatalogTabs() {
        ...
        let html = `<button onclick="nrSetCat('all')" id="nr-tab-all"
            style="padding:.35rem .85rem;border-radius:999px;border:1.5px solid #059669;background:rgba(5, 150, 105,.1);color:#059669;font-weight:800;font-size:.7rem;cursor:pointer;transition:.2s;white-space:nowrap;">
    ...
    function nrSetCat(cat) {
        ...
        document.querySelectorAll('[id^="nr-tab-"]').forEach(btn => {
            ...
            btn.style.borderColor  = isSel ? '#059669' : 'var(--border-color)';
            btn.style.background   = isSel ? 'rgba(5, 150, 105,.1)' : 'var(--bg-card)';
            btn.style.color        = isSel ? '#059669' : 'var(--text-muted)';
        });
        nrRenderCatalog();
    }
    ...
    function nrRenderCatalog() {
        ...
        grid.innerHTML = items.map((item, idx) => {
            ...
            const stockColor = stock > 10 ? '#059669' : stock > 0 ? '#059669' : '#ef4444';
            ...
            return `<div class="nr-catalog-card" data-nr-idx="${idx}"
                style="background:var(--bg-card);border:1.5px solid ${inCart ? '#059669' : 'var(--border-color)'};border-radius:12px;padding:.85rem;cursor:pointer;transition:all .2s;display:flex;flex-direction:column;gap:.4rem;position:relative;${inCart ? 'background:rgba(5, 150, 105,.04);' : ''}">
                ${inCart ? `<div style="position:absolute;top:8px;right:8px;width:18px;height:18px;background:#059669;border-radius:50%;display:flex;align-items:center;justify-content:center;"><svg xmlns='http://www.w3.org/2000/svg' width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='white' stroke-width='3.5' stroke-linecap='round' stroke-linejoin='round'><polyline points='20 6 9 17 4 12'/></svg></div>` : ''}
                ...
                ${catName ? `<div style="font-size:.65rem;font-weight:700;color:#059669;background:rgba(5, 150, 105,.08);padding:1px 6px;border-radius:5px;align-self:flex-start;">${catName}</div>` : ''}
    ...
    function nrRenderCartPanel() {
        ...
        if (nrCartItems.length === 0) {
            ...
            list.innerHTML = nrCartItems.map((item, idx) => {
                const stockColor = item.stock > 10 ? '#059669' : item.stock > 0 ? '#059669' : '#ef4444';
    ...
    function nrBuildReview() {
        ...
        const purpose = document.getElementById('nr-purpose').value.trim();
        const priorityColors = { urgent:'#dc2626', normal:'#059669', low:'#64748b' };
        const usageColors    = { permanent:'#059669', temporary:'#059669' };
            ...
            ${nrCartItems.map(item => {
                ...
                return `<div style="padding:.85rem 1.25rem;border-bottom:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;gap:1rem;">
                    <div>
                        ...
                        ${catName ? `<div style="font-size:.7rem;color:#059669;font-weight:700;">${catName}</div>` : ''}
        ...
        <div style="background:rgba(5, 150, 105,.05);border:1.5px solid rgba(5, 150, 105,.2);border-radius:12px;padding:1rem 1.25rem;display:flex;align-items:flex-start;gap:.75rem;">
            <i data-lucide="info" style="width:16px;color:#059669;flex-shrink:0;margin-top:2px;"></i>
    ...
    async function submitNewReq() {
        ...
        try {
            ...
            if (data.success) {
                ...
                Swal.fire({ icon:'error', title:'Submission Failed', text: data.message || 'An error occurred.', confirmButtonColor:'#059669' });
                btn.disabled = false;
                btn.style.opacity = '1';
                btn.innerHTML = originalHTML;
            }
        } catch (err) {
            Swal.fire({ icon:'error', title:'Network Error', text:'Could not connect to the server. Please try again.', confirmButtonColor:'#059669' });
            btn.disabled = false;
            btn.style.opacity = '1';
            btn.innerHTML = originalHTML;
        }
